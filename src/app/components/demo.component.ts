...
export class DemoComponent {
  ...
  prompt_result: any;
  bPrompt() {
    if (this.regex) this.opt.inputRegex = new RegExp(this.regex);
    else this.opt.inputRegex = null;
    ...
    if (this.inputOptions) {
      ...
      if (
        this.opt.input === 'select' &&
        this.opt.inputOptions &&
        Array.isArray(this.opt.inputOptions)
      ) {
    ...
    this.dialogService
      .prompt(this.opt.title || '请输入新值', this.opt)
      .then((res: any) => {
  ...
  showAlert5() {
    this.dialogService.addDialog(
      AlertComponent,
      { message: 'Click outside only close button' },
      { backdrop: 'static' },
    );
  }
  ...
  showAlert3() {
    this.dialogService.addDialog(
      AlertComponent,
      { message: 'Wait 5 seconds and dialog will be closed automatically' },
      { timeout: 5000 },
    );
  }
  ...
  showAlert4() {
    this.dialogService.addDialog(
      AlertComponent,
      { message: 'Dialog with red backdrop' },
